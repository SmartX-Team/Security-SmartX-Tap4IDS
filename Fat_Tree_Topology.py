...
class HugeTopo(Topo):
    ...
    def createLink(self):
        ...
        logger.debug("Create Core to Core")
        #for x in range(0, self.iAggLayerSwitch, 1):
        self.addLink(self.CoreSwitchList[0], self.CoreSwitchList[1])
        self.addLink(self.CoreSwitchList[0], self.CoreSwitchList[2])
        self.addLink(self.CoreSwitchList[1], self.CoreSwitchList[2])

        logger.debug("Create Core to Agg")
        #for x in range(0, self.iAggLayerSwitch, 1):
        self.addLink(self.CoreSwitchList[0], self.AggSwitchList[0])
        self.addLink(self.CoreSwitchList[1], self.AggSwitchList[0])

        self.addLink(self.CoreSwitchList[0], self.AggSwitchList[1])
        self.addLink(self.CoreSwitchList[2], self.AggSwitchList[1])

        self.addLink(self.CoreSwitchList[1], self.AggSwitchList[2])
        self.addLink(self.CoreSwitchList[2], self.AggSwitchList[2])

        #for x in range(1, self.iAggLayerSwitch, 1):
        ...
        for x in range(0, self.iAggLayerSwitch, 1):
            self.addLink(self.AggSwitchList[x], self.EdgeSwitchList[3*x+0], bw=100)
            self.addLink(self.AggSwitchList[x], self.EdgeSwitchList[3*x+1], bw=100)
            self.addLink(self.AggSwitchList[x], self.EdgeSwitchList[3*x+2], bw=100)
	  #self.addLink(self.AggSwitchList[x], self.EdgeSwitchList[x+2], bw=100)
